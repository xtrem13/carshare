...
import routes from "./demoCars.js"
...
const AllRoutes = (props) => {
  const routeList = routes.map((item, index) => {
    return (
      <TouchableOpacity onPress={() => props.navigation.navigate("Route")} key={index}>
        <CarCard title={item.title} id={item.groupId} size={item.groupSize} />
      </TouchableOpacity>
    )
  })
  ...
  return (
    <>
      ...
      <TouchableOpacity style={styles.addNew} onPress={() => props.navigation.navigate("AddCar")}>
